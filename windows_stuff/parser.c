 ...
 * (http://www.cubebox.bplaced.net/)
 *
 * You're free to use this file in one of your projects.
 *
 * For more informations about what can be parsed, see the README
 * file.
 *
 * Have fun!
 *
...
char *next_line(FILE *stream){
	...
	// compatibily note: works on Linux ('\n') and Windows. ("\r\n")
...
char *search_for_key(char *key, char case_sensitive){
	...
	if(case_sensitive == 0){
		for(i=0; i<__PARSER_KEYS_READ; i++){
			if(stricmp(__PARSER_SEARCH[i]->key, key) == 0)
